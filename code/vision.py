...
def save_keypoints_img_with_bbox_and_indexmap(img,den_map,bbox,index_map,max_index,opt,save_filename,img_alpha=0.6):
    #img  tensor 3*H*W  keypoints tensor num_keypoints*3  
    ...
    index_map=index_map.float()/(max_index-1)
    index_map=index_map.unsqueeze(2)#h*w*1
    index_map=index_map.cpu().numpy()*255
    index_map=index_map.astype(np.uint8)
    vision_index_map=cv2.applyColorMap(index_map, cv2.COLORMAP_JET)
    vision_img=img_alpha*new_img+(1-img_alpha)*vision_index_map
    vision_img=vision_img.astype(np.uint8)

    i=0
    for b in bbox:
        y1,x1,y2,x2=b[0],b[1],b[2],b[3]
        cv2.rectangle(vision_img,(x1,y1),(x2,y2),(0,255,0),2)
        cv2.putText(vision_img,str(i),(int((x1+x2)/2),int((y1+y2)/2)),cv2.FONT_HERSHEY_SIMPLEX,0.75,(0,255,0),1,cv2.LINE_AA,False)
        i+=1
    try:
        cv2.imwrite(save_filename,vision_img)
    except:
...
def save_img_with_bbox(img,bbox,opt,save_filename):
...
def save_map_with_bbox(den_map,bbox,opt,save_filename):
    #img  tensor 3*H*W  keypoints tensor num_keypoints*3   范围[-1,1]
    ...
    for b in bbox:
        y1,x1,y2,x2=b[0],b[1],b[2],b[3]
        cv2.rectangle(vision_map,(x1,y1),(x2,y2),(0,255,0),2)
...
def save_img_with_bbox_different_color(img,bbox,opt,save_filename):
    if not os.path.exists(os.path.split(save_filename)[0]):
        os.makedirs(os.path.split(save_filename)[0])
    h,w=img.size()[1],img.size()[2]
    mean=opt.mean_std[0]
    ...
    for i in range(len(bbox)):
        ...
        cv2.rectangle(new_img,(x1,y1),(x2,y2),color_map[i],2)
...
def save_keypoints_img_with_bbox(img,den_map,bbox,opt,save_filename,img_alpha=0.6):
    #img  tensor 3*H*W  keypoints tensor num_keypoints*3   范围[-1,1]
